...
export class ClaudeAdapter implements ILLMAdapter {
  ...
  async generate(prompt: string, context: AgentContext): Promise<AgentResponse> {
    ...
    try {
      ...
      this.logger.error(`Claude generate error: ${errorMessage}`);
      throw error;
    } finally {
  ...
  async *streamGenerate(prompt: string, context: AgentContext): AsyncGenerator<string> {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      this.logger.error(`Claude streamGenerate error: ${errorMessage}`);
      throw error;
    } finally {
      this.status = AgentStatus.ONLINE;
    }
  ...
  async *executeWithReAct(
    ...
    try {
      ...
      this.logger.log(`Codex executeWithReAct systemPrompt: ${systemPrompt}`);
  ...
  shouldRespond(): Promise<DecisionResult> {
    return Promise.resolve({ should: true, reason: 'Claude adapter always responds', priority: 'medium' });
  }
  ...
  private async logLlmRequest(sessionId: string, agentId: string, payload: Record<string, unknown>): Promise<void> {
    ...
    this.logger.log(`[ClaudeAdapter] LLM request logged to ${dir}/${fileName}`);
  }
